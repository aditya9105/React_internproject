...
// Import Firebase modules (ES Modules, ensure your environment supports this or use <script type="module">)
import {
  initializeApp
} from "https://www.gstatic.com/firebasejs/11.10.0/firebase-app.js";
import {
...
// Expose Firebase utility functions globally
window.firebaseRef = (path) => ref(database, path);
window.firebaseSet = set;
window.firebasePush = push;
window.firebaseOnValue = onValue;
...
// Listen to realtime changes in Firebase "formResponses"
window.listenToFirebase = (callback) => {
  const dbRef = ref(database, "formResponses");
  onValue(dbRef, (snapshot) => {
    const data = snapshot.val() || {};
...
// Confirm deletion dialog and remove entries from Firebase
window.confirmAndDelete = async (ids = []) => {
  if (ids.length === 0) {
    alert("⚠️ Please choose at least one entry to delete.");
    return;
...
// Render table page with pagination
function renderTablePage(entries) {
  const tbody = $("#entryTable tbody");
  tbody.empty();
  ...
  pagedEntries.forEach(entry => {
    ...
    const rowHtml = `
      <tr>
        ...
        <td>${entry.dob}</td>
        <td>${entry.gender}</td>
        <td><img src="${entry.selectedImage}" class="img-thumb" alt="Image"/></td>
        <td><div class="subject-scroll-cell">${subjects}</div></td>
        <td>
          <button class="btn btn-sm btn-primary mr-1" onclick="openUpdateModal('${entry.id}')">Update</button>
          <button class="btn btn-sm btn-danger" onclick="window.confirmAndDelete(['${entry.id}'])">Delete</button>
...
// Stub for openUpdateModal to avoid errors (implement your actual modal logic)
window.openUpdateModal = (id) => {
  alert(`Open update modal for entry id: ${id}`);
};

// Document ready block - initialize everything
$(document).ready(function () {
  // Make sure jQuery, tablesorter, and Bootstrap JS/CSS are loaded in your HTML BEFORE this script

  // Initialize tablesorter on entry table
  $("#entryTable").tablesorter({
    theme: "bootstrap",
    widgets: ["stickyHeaders"],
    headers: {
      8: { sorter: false },  // image column no sorting
      9: { sorter: false },  // subjects column no sorting
      10: { sorter: false }  // action buttons column no sorting
    }
  });
  ...
  // Row checkbox toggle (using delegated event)
  $(document).on("change", ".row-check", function () {
    const id = $(this).data("id");
    if ($(this).is(":checked")) {
      selectedIds.add(id);
  ...
  // Listen for data from Firebase and render initial table
  window.listenToFirebase(entries => {
    allEntries = entries;
    currentPage = 1;
    renderTablePage(allEntries);
  ...
  // Listen for messages to refresh the page (from iframe/modal etc.)
  window.addEventListener("message", function(event) {
    if (event.data === "refreshParent") {
      $("#entryModal").modal("hide");
      setTimeout(() => location.reload(), 500);
